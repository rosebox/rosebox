...
type CalculationData<A extends CalcOperation | void = void> = A extends 'addition'
    ? [LengthPercentage | Calculation<CalcOperation> | Env, LengthPercentage | Calculation<CalcOperation> | Env]
 ...
 * @added 0.1.0
 */
export class Calculation<A extends CalcOperation = CalcOperation> implements RBType<CalculationData<A>> {
    operationType: A
    data: CalculationData<A>
    valueConstructor: Function
    serialize: () => string

    private constructor(data: CalculationData<A>, operationType: A, valueConstructor: Function) {
    ...
    /** @valueConstructor */
    static cdiv(x1: LengthPercentage | Calculation<CalcOperation> | Env, x2: number): Calculation<'division'> {
        return new Calculation([x1, x2], 'division', Calculation.cdiv)
    }

    /** @valueConstructor */
    static cmult(x1: LengthPercentage | Calculation<CalcOperation> | Env, x2: number): Calculation<'multiplication'>
    static cmult(x1: number, x2: LengthPercentage | Calculation<CalcOperation> | Env): Calculation<'multiplication'>
...
const serializeCalculationOperand = (x: Calculation<CalcOperation> | number | LengthPercentage | Env): string =>
    serializeAtomicValue(x)

const serializeCalculation = (x: Calculation): string =>
    `calc(${serializeCalculationOperand(x.data[0])} ${getOpSign(x)} ${serializeCalculationOperand(x.data[1])})`
