 ...
 * @added 0.2.3
 */
export const scale3d = (x: number, y: number, z: number): TransformFunction<'scale3d'> => ({
    type: 'TransformFunction',
    valueConstructor: scale3d,
    value: [x, y, z],
});

export const isScaleX = (x: TransformFunction): boolean => x.valueConstructor === scaleX;

export const isScaleY = (x: TransformFunction): boolean => x.valueConstructor === scaleY;

export const isScaleZ = (x: TransformFunction): boolean => x.valueConstructor === scaleZ;

export const isScale = (x: TransformFunction): boolean => x.valueConstructor === scale;

export const isScale3d = (x: TransformFunction): boolean => x.valueConstructor === scale3d;

export const serializeScaleX = (x: TransformFunction<'scaleSingleAxis'>): string => `scaleX(${x.value})`;

export const serializeScaleY = (x: TransformFunction<'scaleSingleAxis'>): string => `scaleY(${x.value})`;

export const serializeScaleZ = (x: TransformFunction<'scaleSingleAxis'>): string => `scaleZ(${x.value})`;
