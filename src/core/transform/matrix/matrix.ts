...
export const isMatrix = (x: TransformFunction): boolean =>
  x.valueConstructor === matrix

export const isMatrix3d = (x: TransformFunction): boolean =>
  x.valueConstructor === matrix3d

export const serializeMatrix = (x: TransformFunction<'matrix'>): string =>
  `matrix(${x.value.reduce(
    (acc, val, idx) =>
      idx !== x.value.length - 1 ? acc + `${val}, ` : acc + val,
    ''
  )})`

export const serializeMatrix3d = (x: TransformFunction<'matrix3d'>): string =>
  `matrix3d(${x.value.reduce(
    (acc, val, idx) =>
      idx !== x.value.length - 1 ? acc + `${val}, ` : acc + val,
    ''
  )})`
