...
import { isSkewX, serializeSkewX, isSkewY, serializeSkewY, isSkew, serializeSkew } from './skew';
import { isMatrix, serializeMatrix, isMatrix3d, serializeMatrix3d } from './matrix';
import { isPerspective, serializePerspective } from './perspective';

type TransformPropertyValue = TransformFunction | TransformFunction[] | GlobalCssKeyword | 'none';

export const serializeTransformPropertyValue = (x: TransformPropertyValue): string => {
    // @refactor
    ...
    if (isTranslateX(x)) return serializeTranslateX(x as TransformFunction<'translateX'>);
    if (isTranslateY(x)) return serializeTranslateY(x as TransformFunction<'translateY'>);
    if (isTranslateZ(x)) return serializeTranslateZ(x as TransformFunction<'translateZ'>);
    if (isTranslate(x)) return serializeTranslate(x as TransformFunction<'translate'>);
    if (isTranslate3d(x)) return serializeTranslate3d(x as TransformFunction<'translate3d'>);
    if (isScaleX(x)) return serializeScaleX(x as TransformFunction<'scaleSingleAxis'>);
    if (isScaleY(x)) return serializeScaleY(x as TransformFunction<'scaleSingleAxis'>);
    if (isScaleZ(x)) return serializeScaleZ(x as TransformFunction<'scaleSingleAxis'>);
    ...
    if (isRotate3d(x)) return serializeRotate3d(x as TransformFunction<'rotate3d'>);
    ...
    if (isMatrix3d(x)) return serializeMatrix3d(x as TransformFunction<'matrix3d'>);
    if (isPerspective(x)) return serializePerspective(x as TransformFunction<'perspective'>);
...
export { translateX, translateY, translateZ, translate, translate3d } from './translate';
