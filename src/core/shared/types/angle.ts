...
export class Angle<A extends AngleUnit = AngleUnit> implements RBType<number> {
    valueConstructor: (x: number) => Angle<A>
    data: number
    unit: A
    serialize: () => string

    private constructor(data: number, unit: A, valConstructor: (x: number) => Angle<A>) {
        this.unit = unit
        this.data = data
...
export const isAngle = (x: unknown): x is Angle => x instanceof Angle
export const isDeg = (x: Angle): boolean => isAngle(x) && x.unit === 'deg'
export const isGrad = (x: Angle): boolean => isAngle(x) && x.unit === 'grad'
